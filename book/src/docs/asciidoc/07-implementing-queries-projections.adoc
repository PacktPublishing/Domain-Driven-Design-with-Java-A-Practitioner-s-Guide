...
....
<1> This is the dynamic spring data finder method.
<2> The `@QueryHandler` annotation provided by Axon framework routes query requests to the respective handler.
<3> Finally, we invoke the finder method to allow to return results.
...
<2> We execute the query using the `MyDraftLCsQuery` object.
