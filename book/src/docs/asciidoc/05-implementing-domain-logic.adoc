...
In the <<command-query-responsibility-segregation-cqrs>>, we described how DDD and CQRS complement each other and how the command side (write requests) is the home of business logic.In this chapter, we will implement the command side API for the LC application using https://spring.io/projects/spring-boot[Spring Boot]footnote:[https://spring.io/projects/spring-boot] and https://axoniq.io/product-overview/axon-framework[Axon Framework]footnote:[https://axoniq.io/product-overview/axon-framework]. We will also look at how to implement structural and business validations using https://beanvalidation.org/[JSR-303 Bean Validations]footnote:[https://beanvalidation.org/]. and persistence options by contrasting between state-stored vs event-sourced aggregates.
...
A simplified representation of a real-world `LCApplicationCreatedEvent` is shown here:
...
With the above examples, we were able to look at ways to implement the policy enforcements encapsulated within the boundaries the aggregate.
...
1. Can you examine the eventstorming session artifact you had attempted in the last chapter, and identify the possible aggregates that would be required?
...
3. Based on your current understanding, would you apply CQRS architecture pattern? And how would you justify the choice to your team ?
