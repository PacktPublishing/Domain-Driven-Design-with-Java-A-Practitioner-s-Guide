...
|E1: Milk added to cart

|Add white bread to cart
|Order 123: Milk, White bread in cart
|E1: Milk added to cart +
E2: White bread added to cart

|Add milk to cart
|Order 123: Milk(2), White bread in cart
|E1: Milk added to cart +
E2: White bread added to cart +
E3: Milk added to cart

|Remove White bread
|Order 123: Milk(2) in cart
|E1: Milk added to cart +
E2: White bread added to cart +
E3: Milk added to cart +
...
|Add Wheat bread
|Order 123: Milk(2), Wheat bread in cart
|E1: Milk added to cart +
E2: White bread added to cart +
E3: Milk added to cart +
E4: White bread removed from cart +
...
|Confirm checkout
|Order 123: Ordered Milk(2), Wheat bread
|E1: Milk added to cart +
E2: White bread added to cart +
E3: Milk added to cart +
E4: White bread removed from cart +
...
However, there are downsides to persisting data in the form of a simple event log. Before processing any command, we will need to hydrate past events in order of occurrence and reconstruct aggregate state to allow us to perform validations. For example, when confirming checkout, we need to know what exact items are in the cart before allowing the order to be placed. If there are a large number of events, this can be a time-consuming operation directly proportional to the number of past events for that aggregate. There are techniques (like snapshotting) we can employ to overcome this. We will cover this in more detail in a subsequent chapter.
