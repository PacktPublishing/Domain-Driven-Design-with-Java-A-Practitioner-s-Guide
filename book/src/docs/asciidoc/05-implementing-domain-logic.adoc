...
[source,java,linenum]
----
include::{sourcedir}/ch05/LCApplicationAggregateTests.java[lines=13..,indent=0,opts=linenums]
----
...
<3> This is where we instantiate a new instance of the command object. Command objects are usually similar to data transfer objects, carrying a set of information.
<4> Since this is the start of the business process, there are no events that have occurred thus far. This is signified by the fact that we do not pass any arguments to the `given` method. In other examples we will discuss later, there will likely be events that have already occurred prior to receiving this command.
<5> This is where the system receives our command for handling. This command will be routed to our aggregate for handling. We will take a look at how this works in detail shortly.
<6> Here we are declaring that we expect events matching an exact sequence.
<7> Here we are expecting an event of type `LCApplicationCreated` to be emitted as a result of successfully handling the command.
<8> We are finally saying that we do not expect any more events -- which means that we expect exactly one event to be emitted.
...
[source,java,linenum]
----
include::{sourcedir}/ch05/LCApplication.java[lines=12..31,indent=0]
----
...
This was a whirlwind introduction to a simple event-driven system. In subsequent sections and chapters, we will delve deeper into the mechanics of how all of this wired together. Let's now look at how and where we can plug in validations into the system in the upcoming section.
...
The `SubmitLCApplicationCommand` is shown below:
...
....
    ...
    public class SubmitLCApplicationCommand {
        ...
        public SubmitLCApplicationCommand(LCId id) {
            this.id = id;
        }
    }
....

Let's also look at the command handler to implement the validation:
...
class LCApplication {
    ...
    // ...
    @CommandHandler
    private void submit(SubmitLCApplicationCommand command) {
        if (this.state != State.DRAFT) { // <1>
            throw new DomainException("LC is already submitted!");
        }
        apply(new LCApplicationSubmittedEvent(command.getId()));
    }
}
....
<1> Note how we are using the state attribute from the `LCApplication` aggregate to perform the validation.

Let's also look at an example of a validation where information needed to perform the validation is not part of either the command or the aggregate.Country regulations prohibit transacting with a set of other countries -- called the sanctioned countries.Changes to this list of countries may be affected by external factors.In that sense, it does not make sense to pass this list of sanctioned countries as part of the command payload.Neither does it make sense to maintain it as part of the aggregate state.In such a case, the validation can be performed in a manner shown below:

[source,java,linenum]
....
include::{sourcedir}/ch05/CreateLCApplicationCommandHandler.java[lines=7..,indent=0]
....
<1> We are using the `Repository` interface provided by the Axon framework to load the aggregate, and not the one provided by the spring framework's data library.In the <<_persisting_aggregates>> section, we will look at why this is relevant.
<2> The set of sanctioned countries is injected externally.The actual means of maintaining this list is out of scope for the purposes of this book.
<3> As a good practice, we want to perform business validations as close to the aggregate as possible.The actual validation is show in the snippet below.

[source,java,linenum]
....
include::{sourcedir}/ch05/LCApplication.java[lines=33..38,indent=0,opts=linenums]
....
