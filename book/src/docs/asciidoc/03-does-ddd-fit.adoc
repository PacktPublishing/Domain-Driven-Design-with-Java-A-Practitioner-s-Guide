...
In the example above, _place order_ might require us to apply complex business invariants and operate within the purview of an ACID transaction. However, _search orders_ may require us to simply fetch existing data from a query optimized view. As shown here, this style makes use of a horses for courses approach to layering that may help alleviate some anti-patterns we may encounter when implementing a plain vanilla layered architecture.

For example, a very simple classification might be to segregate requests into two broad categories: commands (operations that require state mutations) and queries (operations that merely present information without mutating it). We cover this style of architecture in more detail in the <<_cqrs_pattern,CQRS>> section of this chapter.
