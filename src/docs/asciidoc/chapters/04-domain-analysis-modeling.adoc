...
When working with a problem where domain concepts are unclear, there is a need to arrive at a common understanding among key team members (both those that have bright ideas -- business/product people, and those that translate those ideas into working software -- software developers). For this process to be effective, we tend to look for approaches that are:
...
NOTE: Isnt' the following out of place, as they are not modelling techniques but rather engineering focussed practices to write better code?
* TDD, BDD, FDD, ATDD
